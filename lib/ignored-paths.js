 ...
 */
const normalizePathSeps = path.sep === '/' ? str => str : ((seps, str) => str.replace(seps, '/')).bind(null, new RegExp(`\\${path.sep}`, 'g'));
...
class IgnoredPaths {
  ...
  addPatternRelativeToCwd(ig, pattern) {
    ...
    const cookedPattern = baseDir === this.options.cwd ? pattern : relativize(pattern, path.relative(baseDir, this.options.cwd));
  ...
  addPatternRelativeToIgnoreFile(ig, pattern) {
    const baseDir = this.getBaseDir();
    const cookedPattern = baseDir === this.ignoreFileDir ? pattern : relativize(pattern, path.relative(baseDir, this.ignoreFileDir));
  ...
  contains(filepath, category) {
    ...
    if (typeof category === 'undefined') {
      result = this.ig.default.filter([relativePath]).length === 0 || this.ig.custom.filter([relativePath]).length === 0;
    } else {
